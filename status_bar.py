...
def get_wireless_state():
    ...
    with open("/sys/class/net/wlp3s0/operstate", "r") as f:
...
while True:
    ...
    battery_stat, bat_glyph = get_battery_status()
    wireless_state, wifi_glyph = get_wireless_state()
    t = get_cur_time()


    output_str = f" {wifi_glyph}  {wireless_state} | {bat_glyph}  {battery_stat}% | {t}"
